...
from config import load_config, get_dataloaders, train_model

def main():
    # argparse configuration
    parser = argparse.ArgumentParser(description="Train model with specified configuration.")
    parser.add_argument('--train', action='store_true', help="Flag to initiate training")
    parser.add_argument('--config', type=str, default="config.yaml", help="Path to the YAML configuration file")
    args = parser.parse_args()

    # Load the configuration file
    config = load_config(args.config)
    dataset = ImageMaskDataset(image_dir=config.data_path)
